...
<div id="templatemo_wrapper">
	...
	<table border="1" width="700" style="border-style: inherit; border-color: black;">
		...
		<c:forEach items="${orders}" var="order"  varStatus ="status">
			<tr style="color: grey">
				<td>${status.count}</td>
				<td>${order.ordertime}<%--<fmt:formatDate value="${order.ordertime}" pattern="yyyy-MM-dd HH:mm:SS"/>--%> </td>
				<td>${order.money }</td>
				<td>
				...
				<td>
					<a href="${pageContext.request.contextPath}/user/OrderServlet?op=findOrderDetail&oid=${order.oid}">查看订单详情 </a>|
